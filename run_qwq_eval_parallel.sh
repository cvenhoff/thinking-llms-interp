#!/bin/bash
# QwQ-32B hybrid eval: 3 conditions (learn / rand / biasonly) in parallel,
# one per GPU.  Each condition loads both 32B models on a single H200 (~128GB
# of 143GB).  All use the canonical PG-mid sweep [0.5, 1.0, 1.5] and judge x3.
set -uo pipefail
...
# Launch all 3 in parallel on separate GPUs.
run_one_gpu 0 "qwq-bf-learn-128"     ""                                    &
PID0=$!
run_one_gpu 1 "qwq-bf-rand-128"      "--randomize_vectors --random_seed 42" &
PID1=$!
run_one_gpu 2 "qwq-bf-biasonly-128"  "--bias_only"                          &
PID2=$!
