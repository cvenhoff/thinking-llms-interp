...
num_rollouts = -1
max_examples_per_first_token = 50
...
def update_mean_vectors(mean_vectors, layer_outputs, label_positions, used_counts, index, max_examples=10):
    ...
    for label, positions in label_positions.items():
        ...
        for start, end in valid_positions:
            mean_vector = layer_outputs[:, start-1:start].mean(dim=1)
...
for i, annotated_response_data in tqdm(enumerate(annotated_responses_data), total=num_rollouts, desc="Processing annotated responses"):
    ...
    base_response_data = next((msg for msg in original_messages_data if msg["response_uuid"] == annotated_response_data["response_uuid"]), None)

    prompt_and_model_response_input_ids = prepare_model_input(
        response_uuid=base_response_data["response_uuid"],
