...
# but using LAST-TOKEN-classified annotations (matches eval-time
# classification).  Same hyperparameters as run_orz7b_canon.sh.
set -uo pipefail
...
LAYER=16
SAVE1_FROM_TV="results/vars/correction_vectors_orz7b_biasfirst_stage1_canon_lt"
SAVE2_FROM_TV="results/vars/correction_vectors_orz7b_biasfirst_stage2_canon_lt"
SAVE1="train-vectors/$SAVE1_FROM_TV"
SAVE2="train-vectors/$SAVE2_FROM_TV"
mkdir -p "$SAVE1" "$SAVE2"

LOG1="/tmp/orz7b_canon_lt_stage1.log"
LOG2A="/tmp/orz7b_canon_lt_stage2a.log"
LOG2B="/tmp/orz7b_canon_lt_stage2b.log"
...
if [ ! -f "$SAVE1/qwen2.5-7b_bias_global.pt" ]; then
    echo "===== Stage 1 (LT): collect 20K + train global bias (L${LAYER}, n_epochs=5) ====="
...
if [ ! -f "$SAVE2/disagreements.pt" ]; then
    echo "===== Stage 2a (LT): re-collect 20K under bias ====="
    cd /workspace/thinking-llms-interp/train-vectors
    CUDA_VISIBLE_DEVICES=0,1,2 python -u optimize_correction_vectors.py \
        --base_model "Qwen/Qwen2.5-7B" \
        --thinking_model "Open-Reasoner-Zero/Open-Reasoner-Zero-7B" \
    ...
    echo "Stage 2a (LT) already done."
fi

# ---- Stage 2b ----
echo "===== Stage 2b (LT): train cats with frozen bias (n_epochs=5) ====="
cd /workspace/thinking-llms-interp/train-vectors
CUDA_VISIBLE_DEVICES=0,1,2 python -u optimize_correction_vectors.py \
    --base_model "Qwen/Qwen2.5-7B" \
    --thinking_model "Open-Reasoner-Zero/Open-Reasoner-Zero-7B" \
...
cd /workspace/thinking-llms-interp
run_one_gpu 0 "orz-bf-learn-canon-lt-${N_TASKS}"     ""                                    &
PID0=$!
run_one_gpu 1 "orz-bf-rand-canon-lt-${N_TASKS}"      "--randomize_vectors --random_seed 42" &
PID1=$!
run_one_gpu 2 "orz-bf-biasonly-canon-lt-${N_TASKS}"  "--bias_only"                          &
PID2=$!
