...
# gap to mlp_vectors_qa_instr_holdoutsel_h512/<CONFIG>/ (the vectors every OOD /
...
H="${MLP_HIDDEN}"
DIRS=(
    "${ROOT}/mlp_vectors_qa_instr_h${H}/${CONFIG}"
    "${ROOT}/mlp_vectors_qa_instr_h${H}_bo3/${CONFIG}/run2"
    "${ROOT}/mlp_vectors_qa_instr_h${H}_bo3/${CONFIG}/run3"
)
SELVEC="${ROOT}/mlp_vectors_qa_instr_holdoutsel_h${H}/${CONFIG}"
...
for i, d in enumerate(dirs):
    f = os.path.join(root, "mlp_eval_holdoutmix", cfg, f"run{i+1}", f"judge_reps_{bs}_holdoutmix_final.json")
...
cp -f "${best_dir}/disagree_cache.pt" "${SELVEC}/" 2>/dev/null || \
    ln -sfn "${ROOT}/mlp_vectors_qa_instr_h${H}/${CONFIG}/disagree_cache.pt" "${SELVEC}/disagree_cache.pt" 2>/dev/null || true
