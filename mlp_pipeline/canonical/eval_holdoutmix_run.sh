#!/usr/bin/env bash
# Real hybrid gap-recovery on the trainmix VAL holdout (788 gold Qs:
# hendrycks_math 500 + natural_reasoning 288), used as a vector-SELECTION signal.
# Mirrors the OOD math/gsm8k hybrid setup: qa_instr base prompt (greedy), think
# temp0.6 s0 (re-indexed cache), Claude judge x3. Base rollouts are generated on
# the fly and cached (base_<bs>_holdoutmix_temp0_max2048.jsonl) so sibling runs
# of the same config reuse them.
#
# Env (required): CONFIG, VEC_DIR, RUNTAG
set -euo pipefail
...
echo "== holdoutmix hybrid | CONFIG=${CONFIG} RUNTAG=${RUNTAG} VEC_DIR=${VEC_DIR} | $(date -u) =="
...
[[ -f "${CACHE}/thinking_${TS}_holdoutmix_temp0.6_max2048_s0.jsonl" ]] || { echo "FATAL: missing holdoutmix think cache for ${TS}"; exit 2; }
...
echo "[cfg] steer=${SL} sae=${SAEL} k=${NK} gpus=${N_GPUS} hbs=${HBS}"
...
cd "${ROOT}"
echo "== DONE holdoutmix ${CONFIG}/${RUNTAG} $(date -u) =="
