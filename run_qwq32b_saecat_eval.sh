#!/bin/bash
# QwQ-32B SAE-cat eval at n=128 tasks, judge x3.
# Reuses already-trained vectors in correction_vectors_qwq32b_biasfirst_stage2_saecat.
# Uses GPUs 1+2 (PP via device_map=auto for 32B).
# Includes the multi-GPU mask-device fix in hybrid_eval.py.
...
run_one "qwq-bf-learn-saecat-${N_TASKS}"     ""
run_one "qwq-bf-rand-saecat-${N_TASKS}"      "--randomize_vectors --random_seed 42"
run_one "qwq-bf-biasonly-saecat-${N_TASKS}"  "--bias_only"

echo "ALL QWQ-32B SAE-CAT EVAL DONE."
