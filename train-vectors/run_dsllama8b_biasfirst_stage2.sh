#!/bin/bash
# Stage 2 bias-first for Llama (split into 2a/2b for VRAM hygiene).
set -euo pipefail
...
STAGE1_DIR="results/vars/correction_vectors_dsllama8b_biasfirst_stage1"
BIAS_PATH="$STAGE1_DIR/llama-3.1-8b_bias_global.pt"
if [ ! -f "$BIAS_PATH" ]; then
    echo "ERROR: bias not found at $BIAS_PATH"; exit 1
fi

SAVE_DIR="results/vars/correction_vectors_dsllama8b_biasfirst_stage2"
mkdir -p "$SAVE_DIR"

LOG2A="/tmp/dsl_biasfirst_stage2a_collect.log"
if [ ! -f "$SAVE_DIR/disagreements.pt" ]; then
    ...
    python -u optimize_correction_vectors.py \
        ...
        --frozen_bias_path "$BIAS_PATH" --frozen_bias_layer 19 \
        --collect_only \
        2>&1 | tee "$LOG2A"
else
...
LOG2B="/tmp/dsl_biasfirst_stage2b_train.log"
