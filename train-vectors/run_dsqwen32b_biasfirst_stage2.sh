...
#
# Stage 2a (re-collect) is single-process pipeline-parallel as before.
# Stage 2b (cats training) runs SINGLE-PROCESS PIPELINE-PARALLEL too,
# matching DSQ stage 1; see comment in run_dsqwen32b_biasfirst_stage1.sh
# for the rationale.
set -euo pipefail
...
N_RECOLLECT=${DSQ_N_RECOLLECT:-3000}
...
echo "===== Stage 2b: training cat vectors with frozen bias (single-proc PP) ====="
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
python -u optimize_correction_vectors.py \
    --base_model "Qwen/Qwen2.5-32B" \
    --thinking_model "deepseek-ai/DeepSeek-R1-Distill-Qwen-32B" \
